...
PIUMETTA_FOTO = "D:/claude_handoff/outbox/Roma_pesca_campionato_2026/esche_foto/piumetta_prod.jpg"  # foto-prodotto Williamson Flash Feather (c) Williamson/Rapala, uso non-commerciale guida squadra

fig, axs = plt.subplots(2, 2, figsize=(11, 7.2))
fig.suptitle("Esche dell'assetto  -  3 foto reali (minnow, kona, piumetta) + 1 schema (teaser)",
...
# --- 3) PIUMETTA / FEATHER  [FOTO REALE] ---
ax = axs[1, 0]; ax.set_title("PIUMETTA / feather (piombata)  [FOTO REALE]", fontsize=11, fontweight="bold")
if os.path.exists(PIUMETTA_FOTO):
    ax.imshow(mpimg.imread(PIUMETTA_FOTO), extent=[0.05, 0.95, 0.30, 0.80], aspect="auto", zorder=1)
else:
    ax.text(0.5, 0.5, "(foto non trovata)", ha="center")
...
fig.text(0.5, 0.005,
         "Foto: minnow (Wikimedia Commons, CC0) - kona/octopus (Lonnie's Life, Flickr, CC BY-NC 2.0) - "
         "piumetta (Williamson Flash Feather, foto prodotto (c) Williamson/Rapala, uso non commerciale squadra). "
         "Teaser = schema (daisy-chain: nessuna foto libera pulita). Verificate: minnow ~13 cm + piumette piombate [R7]; "
